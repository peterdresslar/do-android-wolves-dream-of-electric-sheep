...
def load_environment():
    ...
    if not _env_loaded:
        ...
        if os.path.exists(dotenv_path):
            ...
            print(
                f"Warning: Could not find .env.local file. Tried paths: {os.path.join(os.path.dirname(__file__), '..', '.env.local')}, {os.path.join(os.getcwd(), '.env.local')}, {os.path.join(os.getcwd(), '..', '.env.local')}"
            )
            return False
